-- PostgreSQL Migration: Atomic Server-Side Payroll Processing RPC
-- Handles workdays count, LOP checks, progressive tax bracket calculations and payslip inserts atomically in a database transaction.
    ...
    LOOP
        ...
        LOOP
            ...
            IF v_weekday = ANY(v_allowed_work_days) AND NOT v_is_holiday THEN
                ...
                -- Unpaid criteria: code is 'LWP' or name contains 'unpaid', 'loss of pay', 'without pay'
                SELECT EXISTS (
                    SELECT 1 
        ...
        v_monthly_gross := v_base_monthly_gross * v_proration_factor;
        ...
        DECLARE
            ...
            v_ded_round numeric(12,2) := ROUND((v_income_tax_monthly + v_statutory_deductions_monthly) * 100) / 100;
            v_net_round numeric(12,2) := ROUND((v_monthly_gross - (v_income_tax_monthly + v_statutory_deductions_monthly)) * 100) / 100;
            v_breakdown jsonb;
        BEGIN
