...
## 2025-05-12 - Hardcoded Supabase Credentials
**Vulnerability:** Supabase URL and Publishable Key were hardcoded in plaintext in `src/integrations/supabase/client.ts` despite the file containing a "do not edit" comment.
**Learning:** Automatically generated configuration files may contain sensitive hardcoded credentials depending on the tooling that generated them.
**Prevention:** Use environment variables (e.g., `import.meta.env.VITE_SUPABASE_URL`) for all credentials even in auto-generated files, overriding the tooling defaults if necessary.
