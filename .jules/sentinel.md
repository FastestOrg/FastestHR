...
## 2024-05-20 - [Fix ReDoS Vulnerability in Template Variable Subsitution]
**Vulnerability:** Regular Expression Denial of Service (ReDoS) vulnerability due to passing user-provided or dynamic keys directly into `new RegExp` constructors in `IDCardTemplateEditor.tsx`, `PublicIDCard.tsx`, `VirtualIDCard.tsx`, `OfferLetterRenderer.tsx`, and `DocumentRenderer.tsx`. Unescaped special characters could be used to construct a slow regular expression that could crash the application.
**Learning:** A common anti-pattern is replacing keys in templates by dynamically generating regular expressions without escaping the string first. This leaves the string evaluation susceptible to ReDoS when malicious tokens trigger catastrophic backtracking.
**Prevention:** Always escape regex special characters (e.g., using `key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')`) prior to creating a dynamic `RegExp`.
