...
## 2024-05-18 - [Fix Protocol-based XSS in Author Details]
**Vulnerability:** In `src/pages/AuthorDetail.tsx`, the application was directly using user-provided social media URLs (`author.linkedin` and `author.twitter`) in the `href` attribute of `<a>` tags without prior validation. This could allow protocol-based XSS attacks where an attacker inputs a URL starting with `javascript:` to execute malicious scripts when a user clicks the link.
**Learning:** React escapes HTML content automatically to prevent XSS in child nodes, but it does not sanitize or validate URIs in attributes like `href`. All user-supplied URLs must be explicitly validated to ensure they use a safe protocol. This aligns with a previous finding, highlighting a recurring pattern that needs strict enforcement.
**Prevention:** Always implement a validation check using a utility function like `isSafeUrl` (which ensures URLs start with `http://` or `https://`) before rendering them in an anchor tag attribute.
