...
## 2025-02-28 - [CRITICAL] Fix Regex Injection/ReDoS in Template Renderers
**Vulnerability:** Unescaped string variables (keys) were being passed directly into `new RegExp(key, 'g')` across multiple template rendering components (OfferLetterRenderer, DocumentRenderer, IDCardTemplateEditor). This allowed for Regex Injection and potential Regular Expression Denial of Service (ReDoS).
**Learning:** The codebase heavily relies on dynamic string replacement for variables like `{{key}}`. When generating regular expressions from these untrusted or dynamic keys, failure to escape special characters allows manipulation of the regex engine.
**Prevention:** Always use `key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')` to escape literal strings before passing them to the `RegExp` constructor.
