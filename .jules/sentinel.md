...
## 2025-10-27 - [Fix Regex Injection in RegExp constructor]
**Vulnerability:** The application was dynamically creating `RegExp` objects using user-provided variable keys without escaping regex metacharacters (`new RegExp(key, 'g')`). This allowed an attacker to supply a template key with regex special characters (like `.*`, `+`, `?`, `^`, `$`, etc.) resulting in Regex Injection and potential Regular Expression Denial of Service (ReDoS) or unintended string substitutions.
**Learning:** When generating `RegExp` objects from dynamic or untrusted strings, the string may be parsed as a regular expression pattern rather than a literal string, allowing attackers to manipulate the logic or cause performance issues via catastrophic backtracking.
**Prevention:** Always escape regex metacharacters by using a replacement like `key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')` before passing the dynamic string to the `RegExp` constructor.
