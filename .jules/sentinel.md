...
**Vulnerability:** The `src/integrations/supabase/client.ts` file contained hardcoded `SUPABASE_URL` and `SUPABASE_PUBLISHABLE_KEY` values directly in the source code, exposing these credentials to anyone with access to the source code or build artifacts.
**Learning:** Even files marked as "automatically generated" should be reviewed for security issues like hardcoded credentials. It is crucial to ensure that environment variables are correctly used instead of inline secrets.
**Prevention:** Always use environment variables (`import.meta.env.VITE_SUPABASE_URL`, etc.) for sensitive configuration values. Setup automated linting or pre-commit hooks to scan for known credential patterns (e.g., using tools like detect-secrets or trufflehog) to catch these before they are committed.
