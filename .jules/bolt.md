...
## 2024-05-25 - Analytics Multi-Pass Iteration Bottlenecks
**Learning:** Performing multiple independent `.filter()` and `.forEach()` iterations on the same large array during a React render cycle (e.g., to derive various aggregate statistics) wastes CPU time and creates performance overhead as datasets grow.
**Action:** Combine multiple aggregation processes into a single `useMemo` block that executes a single pass (using `.forEach` or `.reduce`) over the dataset to compute all required analytics at once.
