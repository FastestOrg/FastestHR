...
## 2024-12-05 - Extract Invariant Array.find from Map Loops
**Learning:** In `src/pages/KPI.tsx`, mapping over an array in the render function and calling `Array.find()` to locate a corresponding element creates an O(N*M) bottleneck, degrading performance.
**Action:** Extract the search logic by utilizing `useMemo` to build an O(1) dictionary (`Record<string, T>`) ahead of the map, replacing the O(N) inline search with an O(1) property lookup (`dictionary[id]`).
