...
## 2026-05-31 - Memoizing loop calculations\n**Learning:** O(N) array calculations inside components that perform render-cycle timers (e.g. `setInterval` updating state every second) run repeatedly causing performance bottlenecks.\n**Action:** Use `useMemo` and precalculate loop values into dictionaries to avoid O(N*M) reduction loops inside render cycles that update frequently.
